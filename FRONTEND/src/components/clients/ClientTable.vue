<template>
  <section class="panel">
    <header class="panel__header">
      ...
      <RouterLink v-if="props.canCreate" class="primary-button primary-button--compact primary-button--accent-soft" to="/clientes/novo">Novo +</RouterLink>
    </header>

    <div class="filters-grid">
      <input v-model="draft.nome" class="field" placeholder="Pesquisar por nome" type="text" />
      <input v-model="draft.cpf_cnpj" class="field" placeholder="Filtrar por CPF/CNPJ" type="text" @blur="formatFilterDocument" />
      ...
      <table class="data-table data-table--cadastro">
        ...
        <tbody>
          ...
          <tr v-for="client in props.result.items" :key="client.clientes_id" class="data-table__row">
            ...
            <td>{{ client.nome || '-' }}</td>
            <td>{{ formatDocument(client.cpf_cnpj) }}</td>
            <td>
              <span class="cellphone-display">
                <span v-if="client.flag_whatsapp && client.celular01" class="cellphone-display__whatsapp" aria-label="WhatsApp" title="WhatsApp">
                ...
                <span>{{ formatPhone(client.celular01) }}</span>
              </span>
            </td>
            ...
            <td class="actions-cell">
              <button v-if="props.canUpdate" class="icon-action" type="button" title="Alterar cadastro" aria-label="Alterar cadastro" @click="$emit('edit', client.clientes_id)">
                <svg viewBox="0 0 24 24" aria-hidden="true">
              ...
              <button v-if="props.canDelete" class="icon-action icon-action--danger" type="button" title="Excluir cadastro" aria-label="Excluir cadastro" @click="$emit('delete', client.clientes_id)">
                <svg viewBox="0 0 24 24" aria-hidden="true">
            ...
            <td class="actions-cell">
              <button class="icon-action icon-action--message" type="button" title="Gerar novo contrato" aria-label="Gerar novo contrato" @click="$emit('contract', client.clientes_id)">
                <svg viewBox="0 0 24 24" aria-hidden="true">
...
const emit = defineEmits<{
  apply: [payload: { nome?: string; cpf_cnpj?: string; ativo?: boolean }]
  'change-page': [page: number]
  'change-page-size': [pageSize: number]
  edit: [clientId: number]
  delete: [clientId: number]
  contract: [clientId: number]
}>()
...
watch(
  ...
  (filters) => {
    ...
    draft.cpf_cnpj = filters.cpf_cnpj ?? ''
